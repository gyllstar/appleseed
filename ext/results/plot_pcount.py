...
def plot_loss_rate(data_file,base_loss_ratio,sd_flag=False):
	...
	data = np.genfromtxt(rate_folder + data_file,data_file, delimiter=',',names=['w', 'mf', 'samples','mean_time','ci1_time','ci2_time','mean_loss','ci1_loss','ci2_loss']) 
	...
	for i in range(1,max_mf+1):
		...
		plt.plot(x,y, marker='o',linestyle='None',color=colors[indx])#,label="f=%i" %(i))
